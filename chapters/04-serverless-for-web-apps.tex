...
AWS Lambda execution model fits into the mentioned classification and distinguish the following types of function invocation \cite{OptimiseYourServerlessApplications}.

rb: TBD
% ref to the example - \label{chapter:examples-receipt-processing}

\begin{itemize}
    \item synchronous --- 
    % push based
    % API Gateway, making request to the API, which in turn invokes the lambda function, while lambda responds back to the API with some sort of response, passed back to the client

    \item asynchronous --- 
    % event based
    % message put on the SNS topic or image is put in the S3 bucket
    % create a request, going out to the lambda function
    % consuming the event, no way to pass back the information to originated client or service if the work is done
    % needs to be handled b by the application if we want to respond back to the user

    \item stream --- 
    % poll based model
    % Amazon DynamoDB, Kinesis Datastreams, SQS
    % Lambda service runs a poller which watches the service for new messages, takes the data out and passes to the Lambda functions via the asynchronous event
