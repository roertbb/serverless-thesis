...
text is still work in progress - but ideas could be worth sharing 
...
Exercise details - different overhead for the latex processing
...
% TODO:rb - how long it takes to render these on personal computer
...
\pgfplotstableread{
% xaxis, Layer Cold, Layer Warm, Image Cold, Image Warm
0.125 5.455 2.147 7.943 2.209
0.375 9.469 5.235 10.832 5.210
% 2 2 3 4 5
% 3 2 3 4 5
}\datasetLayerVsImage
...
\addplot[draw=black,fill=blue!30,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=1] \datasetLayerVsImage; % Layer Cold
\addplot[draw=black,fill=red!30,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=2] \datasetLayerVsImage; % Layer Warm
\addplot[draw=black,fill=blue!60,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=3] \datasetLayerVsImage; % Image Cold
\addplot[draw=black,fill=red!60,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=4] \datasetLayerVsImage; % Image Warm
...
\paragraph{Reducing the function chain length}
...
the problem --- chaining several functions --- compound of the overhead related with the communication and cold starts of the functions --- it could be more effective from the performance point of view to implement the whole processing logic inside 1 function instead of 2. Additionally, when there are 2 functions, the intermediate result in form of the PDF file needs to be stored in the external component (S3 in that case) and be retrieved by the second function)

\pgfplotstableread{
% xaxis, Single Cold, Single Warm, Chain Cold, Chain Warm
0.125 6.242 2.724 7.084 2.970
0.375 10.039 5.731 11.612 5.963
% 2 2 3 4 5
% 3 2 3 4 5
...
\addplot[draw=black,fill=blue!30,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=1] \datasetChainVsSingle; % Single Cold
\addplot[draw=black,fill=red!30,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=2] \datasetChainVsSingle; % Single Warm
\addplot[draw=black,fill=blue!60,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=3] \datasetChainVsSingle; % Chain Cold
\addplot[draw=black,fill=red!60,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=4] \datasetChainVsSingle; % Chain Warm
...
\pgfplotstableread{
% xaxis, Single Cold, Single Warm, Parallel Cold, Parallel Warm
0.125 6.242 2.724 4.996 2.136
0.375 10.039 5.731 7.477 3.186
% 2 2 3 4 5
% 3 2 3 4 5
...
\addplot[draw=black,fill=blue!30,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=1] \datasetSingleVsParallel; % Single Cold
\addplot[draw=black,fill=red!30,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=2] \datasetSingleVsParallel; % Single Warm
\addplot[draw=black,fill=blue!60,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=3] \datasetSingleVsParallel; % Parallel Cold
\addplot[draw=black,fill=red!60,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=4] \datasetSingleVsParallel; % Parallel Warm
\legend{Single function - cold start, Single function - warm start, Parallel execution - cold start, Parallel execution - warm start}
\end{axis}
\end{tikzpicture}
...
\begin{itemize}
    ...
    \item parallel processing is limited by the task that takes the longest
...
0.125 6.242 2.724 4.996 2.136 4.315 4.680 1.710 2.718
0.375 10.039 5.731 7.477 3.186 5.611 9.755 1.803 5.814
% 2 2 3 4 5
% 3 2 3 4 5
...
\addplot[draw=black,fill=blue!80,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=5] \datasetUploadInitialBatch; % FirstBatch Cold
\addplot[draw=black,fill=blue!100,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=6] \datasetUploadInitialBatch; % RestBatch Cold
\addplot[draw=black,fill=red!80,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=7] \datasetUploadInitialBatch; % FirstBatch Warm
\addplot[draw=black,fill=red!100,error bars/.cd,y dir=both,y explicit] 
    table[x index=0,y index=8] \datasetUploadInitialBatch; % RestBatch Warm
\legend{Single function - cold start, Single function - warm start, Parallel execution - cold start, Parallel execution - warm start, First Batch - cold start, Remaining Batch - warm start, First Batch - cold start, Remaining Batch - warm start}
\end{axis}
\end{tikzpicture}
...
\begin{itemize}
    \item from the user experience perspective --- the best performance --- delivering and displaying the initial batch as soon as possible, while processing the remaining part, delivering it and updating the client state once its completed
    \item Remaining batch time is longer than the Single function - on one hand there is less work to be done, but on the other there is there could be additional overhead orchestration overhead and there is more work to be done --- defining several tasks, adding the directive
    \item TODO:rb - parallel processing of the remaining batch
    \item TODO:rb - container - how long it takes without serverless architecture - shared infrastructure - simple NodeJS server doing the processing
\end{itemize}

% \paragraph{Lambda params power tunning}

% % TODO:rb add it later in the server tier description
...
\subsection{Serverless processing types}
