...
% depending on the application type and workflows it needs to perform, the proessing may differ from simple requests based on a handful of services as well as more complex setup, hiding a group of microservices working together to perform the application logic

\subsubsection{Synchronous requests}
% works for simple cases when the lambda can make a request to downstream service and return back, mentioned in section ...
% for more complex workflows the processing should go async - lambda can invoke the processing, return with the status to client, polling for the status/results periodically once they are finished
% using rest, similarly to the other

\subsubsection{GraphQL API}
% \cite{ServerlessArchitecturalPatternsAndBestPractices}
% use AppSync to host the GraphQL APIs
% - simpler logic can be handled by resolvers directly communicating with the data sources such as DynamoDB to query the data - insert, listing - simple VTL to connect to the datasource
% - for more complex business logic use lambda functions, perform the processing and return the value to the client
% - pipeline resolvers - performing chainy actions - call service number 1, call service number 2, invokes some processing, booking - pipeline resovlers can do that, but are not prepared for handling the errors and retries effectively
% - use Step Functions for more complex processing, when handling errors and retries is required
% - Cognito / any JWT - appsync allows to set the auth rules per field level - some users/groups can have access to fields, while others not
% \cite{DevelopServerlessGraphQLArchitecturesUsingAWSAppSync}
% - beneficial when working on larger products in a team when multiple teams developing multiple microservices - the logic can be extracted and merge on the GraphQL API level in one place, serving as the routing layer
% - teams can swap the underlying implementation of the services, change the data stores, but as long as the API contract is maintained, it will work - change and evolve the underlying architecture until the requirements are not clarified
% - moreover, leveraging the subscriptions enable developers to update the clients in real time, based on the interactions of other users, change in the underlying data or anything triggered from the backend

\subsubsection{Working with files securely --- S3 presigned URLs}
% \cite{ServerlessAtScaleDesignPatternsAndOptimizations}
% use IAM user/role permissions to create the URL to get/post for a given S3 object
% URL valid for ss long as credentials are valid, or specified expiration time is exceeded
% it can be used for the clients to operate directly with S3 to upload the files or get the files in a secure way, when the access is authenticated/authorized
% Lambda invocation payload (request and response) - 6 MB for synchronous requests
% API Gateway has a payload limit up to 10 MB 

\subsubsection{Webhooks}
% \cite{ServerlessAtScaleDesignPatternsAndOptimizations}
% callback pattern, calling back to http endpoint posted by the caller, to let them know that the work is done
% the client is trusted - verifying some API key or validated some other way
% the client request is submitted to the queue to be processed asynchronously
% api gateway returns as soon as the request is put into the SQS, durably stored
% lambda does it work
% the backend service calls back to the client when work os completed - sns topic/subscription filters to notify the client further
% no need to pooling, but host needs to host a webhook, document retry policy, for untrusted client

\subsubsection{From poll to push --- WebSockets}
% \cite{ServerlessAtScaleDesignPatternsAndOptimizations}
% using both api gateways - rest and websockets
% client submits a request, starting the step function execution, performing some more sophisticated workflow, obtains execution id
% later reaches out to the websocket endpoint, with step function ARN and execution id (or token), establish the connection on websocket
% once step function completes - when the work is done and the client is listening - then we'll let them know once the processing is completed (parallel step - work and connectiong - to make sure that when the work ends quickly, we'll wait until the client connects), once both steps are done we're dsending response back to the client
% similar approach was used in the \ref{chapter:examples-generating-interactive-slideshow-based-on-latex-files} example implementation to notify the client once the processing is done, additionally using the dynamodb to store the executionId/connectionId pair to be lated used by the step function
% open connection to the clients - we can notify them using the open conenctions, richer eventing for web and mobile clients
% requires the knowledge of WebSocketsAPI, 500 new connection per second limit

% use cases made by the community including
% - url shortener without lambda functions, performing the logic with direct integration between API Gateway and S3
% - simple, turn based games based on the WebSocket connection
% - incorporate protocol buffers to reduce the bandwidth
...
% \begin{itemize}
%     \item direct access to the cloud services - Firebase, Amplify
%     \item sometimes it is the only way, however the receipt processing application client is built in an agnostic way, underlying implementation could be replaced with traditional GraphQL server
%     \item the presentation processing flow includes additinal logic, handling the websocket, performing some orchestration/coordination of the application logic
% \end{itemize}
