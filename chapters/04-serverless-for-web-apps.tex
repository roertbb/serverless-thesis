...
It requires a shift in the mental model from the relational modeling to approaches such as single-table design to store multiple, heterogenous item types in one table to query and retrieve them with prejoined data in one request \cite{SQLNoSQLandScaleHowDynamoDBScalesWhereRelationalDatabasesDont}.
...
pipeline resolvers
% https://aws.amazon.com/blogs/mobile/appsync-pipeline-resolvers-2/?sc_channel=sm&sc_campaign=Mobile_Campaign,DB_Blog&sc_publisher=TWITTER&sc_country=DynamoDB&sc_geo=GLOBAL&sc_outcome=awareness&trk=ddbawsappsyncpipelineresolvers_061021_TWITTER&sc_category=Amazon+DynamoDB&linkId=117305496
