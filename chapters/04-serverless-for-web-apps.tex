...
\paragraph{Runtime and data restrictions}
...
\subsection{Serverless processing types/patterns}
...
Reference to the use case with video processing - \cite{ServerlessArchitectureOnAWSSecondEdition} \label{chapter:serverless-processing-effortless-parallelisation}
...
\subsection{Serverless function as a compute resource}

% https://betterdev.blog/aws-lambda-performance-optimization/
...
Lambda params power tunning

% proportionally to the amount of memory the other parameters such as CPU or network bandwidth are assigned
% the latex compiling seems to be not su much memmory consuming, cause the logging most frequently indicate that no more than 200 MB of memory has not been used for the processing

custom - docker image, lambda layer \label{chapter:lambda-custom-runtimes}
...
% TODO: more intro about why step function?
...
\begin{itemize}
   ...
   \item Lambda Layer --- includes the LaTeX distribution extracted from the aforementioned container image into the separate archive along with the additional Lambda Layer including the Perl interpreter.
\end{itemize}
...
\begin{itemize}
    \item Cold start times --- For the solution using the Lambda Layer, the cold start time for the Lambda instance performing the presentation generation task is equal on the average to 273 ms and 279 ms for Exercise 1 and Exercise 2 accordingly.
   For the processing using the Container Image solution, the cold start time is on the average equal to 1108 ms and 810 ms, which has a visible impact on the overall processing time. Moreover, the cold start time of the solution based on the Container Image is additionally billed accordingly to AWS Lambda pricing, on the contrary to the approach using the Lambda Layer in which the cold start time is not included into the cost.
   \item Warm start times --- When considering the prewarmed function environments, there is no visible difference in the overall processing time.
   ...
   However, when comparing the task duration from the Step Function logs and the logs related to the AWS Lambda executions for the presentation processing tasks, some additional overhead was noticed for the cold start invocations.
   The additional time required to execute the function can be related to the resource provisioning and establishing the communication between the orchestrator and the serverless function itself.
   The difference between the function execution time and task duration is negligible in the case of warm starts. The results are summarised in the tables \ref{table:overhead-of-the-presentation-processing-task-for-the-lambda-layer} and \ref{table:overhead-of-the-presentation-processing-task-for-the-lambda-container-image}.

    \begin{table}[h]
        \centering
        \begin{tabular}{ |c|c|c|c|c|c| } 
        \hline
        Exercise & Execution & Task [ms] & Function [ms] & Cold start [ms] & Task overhead [ms] \\
        \hline
        \multirow{2}{*}{Exercise 1} & cold start & 3609 & 2422 & 273 & 914 \\
        & warm start & 1614 & 1580 & 0 & 34 \\
        \hline
        \multirow{2}{*}{Exercise 2} & cold start & 7802 & 7055 & 279 & 468 \\
        & warm start & 4661 & 4609 & 0 & 52 \\
        \hline
        \end{tabular}
        \caption{Overhead of the presentation processing task for approach using Lambda Layer}
        \label{table:overhead-of-the-presentation-processing-task-for-the-lambda-layer}
    \end{table}
    ...
    \begin{table}[h]
        ...
        Exercise & Execution & Task [ms] & Function [ms] & Cold start [ms] & Task overhead [ms] \\
        \hline
        \multirow{2}{*}{Exercise 1} & cold start & 6548 & 4803 & 1108 & 637 \\
        & warm start & 1638 & 1593 & 0 & 45 \\
        \hline
        \multirow{2}{*}{Exercise 2} & cold start & 8945 & 7819 & 810 & 316 \\
        & warm start & 4614 & 4579 & 0 & 35 \\
        \hline
        \end{tabular}
        \caption{Overhead of the presentation processing task for approach using Lambda Container Image}
        \label{table:overhead-of-the-presentation-processing-task-for-the-lambda-container-image}
    \end{table}
\end{itemize}
...
To answer that question for the given problem, a similar service based on a simple server running within the container was created and hosted on the AWS Fargate.
...
\begin{itemize}
   ...
   \item Re-designing the serverless processing to utilise its benefits can improve performance --- The presented configurations of the serverless solution confirms that remodeling the processing to use the benefits of the serverless paradigm can bring visible benefits in terms of the performance, compared to the solution using the traditional architecture.
   One of the features of the serverless architecture is the possibility of almost instant and effortless processing parallelisation, that reduces the overall processing time in that case.
...
\begin{table}[H]
    ...
    \cline{2-3}
    \end{tabular}
    \caption{Pricing summary for the approach using parallel presentation processing}
...
\begin{table}[H]
    ...
    \cline{2-3}
    \end{tabular}
    \caption{Pricing summary for the approach delivering the update to the client in the separate batches}
    \label{table:case-study-service-initial-batch-spending-summary}
\end{table}

% TODO:rb

% It's hard to calculate the cost of such a service, taking into account multiple components taking part in the entire processing flow.
